...
For reference, `--lib` counts per bundle: `tier-0` 65, `tier-1` 106, `tier-2` 146,
`tier-3`/`tier-4` 186, `tier-2,webview` 147, `tier-3,webview` 224,
...
In-process EVM: no network, no `.env`. 295 tests across seven files.
