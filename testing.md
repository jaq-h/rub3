...
Two things only this suite can check, and both are why it exists:
...
- `code_registry_answers_the_wrapper_over_a_real_chain_e2e` - publishes a release through `cast send` using the masked hash and immutable ranges out of `attest::CANONICAL` itself, then asserts `offsetTables()` round-trips those ranges exactly, `consult_registry` returns the record with every field intact, code nobody published is `Unknown`, a `deprecate` reaches the wrapper as `Deprecated` with the record still whole, and a licence contract at the registry's place is never believed as the registry.
