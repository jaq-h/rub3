...
```bash
# tier-3 (adds onchain-write + cooldown): 179 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3 --lib

# tier-3 + the headless (agent) front door: 245 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,headless --lib

# tier-3 + the webview (human) front door: 217 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,webview --lib
```

For reference, `--lib` counts per bundle: `tier-0` 65, `tier-1` 105, `tier-2` 145,
`tier-3`/`tier-4` 185, `tier-2,webview` 146, `tier-3,webview` 223,
`tier-3,headless` 251, `tier-0,sdk` 77, `tier-3,sdk` 202. Every count on this
page is the number libtest reports as *running* - passed plus `#[ignore]`d, not
passed alone - so a plain run of `tier-3,webview` prints 216 passed and 7 ignored
against the 223 above. The five `packed` tests are in every bundle, which is why
every bundle is five higher than the one `implementation.md` §3.5 recorded when
it was written; §3.4's five seat-teardown store tests are five higher again
...
- `a_multi_seat_licence_offers_no_auto_detect_tab_e2e` (§3.4) - above one seat the cooldown screen's payload carries no `autoWatchSecs`, which is what the page reads to decide the tabs, so Manual is the only one offered: the watch resolves an activation from the chain alone, and a fleet instance activating the same token in the same block puts a log in that filter which nothing separates from the person's own, since every instance signs with the holder's key. The manual path is driven to a verified session in the same test
...
- `headless_a_planted_session_file_cannot_release_another_instances_seat_e2e` (§3.4) - the session directory is user-writable and `load_session` is a bare read and parse, so a hand-written record naming this contract and *another instance's* session id would otherwise make the wrapper sign and broadcast `release()` and end that session. Both teardown doors are driven against four forgeries - one internally consistent but signed by a key this run does not hold, which a signature check alone would admit; one whose signature is not a signature; one this machine really wrote on another deploy with `contract` rewritten to name this one, which only `contract` being in the signed preimage catches; and one it really wrote for this contract's address on another chain, which verifies exactly as signed and is refused as `AnotherDeploy` on its `chain_id` by the named-token door and never selected by the scan - and each must sign nothing at all and leave the victim's seat live. The honest release runs at the end of the same test, so the refusals are about who wrote the record rather than about refusing everything
...
In-process EVM: no network, no `.env`. 290 tests across seven files.

- **`test/Rub3Access.t.sol`** (30) - metadata, constructor validation, purchase (including the exact-payment rule: under, over, exact, and a 256-run fuzz proving the listed price is the only accepted amount), supply cap, activation and cooldown, owner gating
- **`test/Rub3Invariants.t.sol`** (41) - the ownership invariants, in four groups: 17 on the append-only hash set (constructor seeding, zero/duplicate rejection, older releases staying valid, revocation status/reason/events, revoked hashes not resurrectable, owner gating), 14 on the successor pattern (opt-in on both sides, one claim per token, the claim following the current holder, survival of renounced ownership, and the trust rule surviving a successor repoint), 4 on mint ordering and the predecessor probe (a `MintCallbackProbe` recipient reading its own `wasClaimed` provenance from inside `onERC721Received` on the claim path, the one mint path that writes per-token state before minting, and `IncompatiblePredecessor` against a codeless and a non-licence predecessor with a well-typed one completing a claim end to end), and 6 on the no-revocation audit (30 forbidden signatures × 3 deployed contracts, the third being the §2.3 `Rub3Factory`, with a positive control proving the scanner finds selectors that do exist - including `feeBps()` and `treasury()`, the getters that exist while every setter for them is absent, plus the two seat audits of §3.4: every seat-shaped call an owner might reach for hitting the missing fallback against a live fleet with `release` refusing the contract owner, and `seatsPerToken` / `sessionTtlSeconds` reading as deployed with the getters present and no setter for either in the bytecode)
...
- **`src/` unit tests** (18) - `deployments::tests` reads the *committed*
  ...
  rounded, the wrapper-hash shape including the zero hash the contract reads as
  "unknown", and `the_plan_clears_every_script_variable_it_did_not_set_itself`,
  which resolves a real plan, pollutes this process's own environment with a
  `SEATS`, a `FACTORY` and a `PRICE`, and asserts over the `forge` invocation
  the plan produces that every name in `SCRIPT_VARS` is either the value this
  command decided or removed outright - an inherited one would be a deploy input
  nobody typed, and it would be silent. Whether `SCRIPT_VARS` *covers* every
  variable `Deploy.s.sol` reads is a separate property and is not mechanised:
  a name missing from the list is still silent, and closing that is its own
  piece of work; `repo::tests` covers checkout resolution
