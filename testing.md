...
```bash
# tier-3 (adds onchain-write + cooldown): 132 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3 --lib

# tier-3 + the headless (agent) front door: 196 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,headless --lib

# tier-3 + the webview (human) front door: 148 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,webview --lib
```

For reference, `--lib` counts per bundle: `tier-0` 52, `tier-1` 82, `tier-2` 122,
`tier-3`/`tier-4` 132, `tier-2,webview` 123, `tier-3,webview` 148,
`tier-3,headless` 196, `tier-0,sdk` 60, `tier-3,sdk` 145. Each total includes
