...
```bash
# tier-3 (adds onchain-write + cooldown): 127 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3 --lib

# tier-3 + the headless (agent) front door: 191 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,headless --lib

# tier-3 + the webview (human) front door: 143 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,webview --lib
```

For reference, `--lib` counts per bundle: `tier-0` 51, `tier-1` 81, `tier-2` 117,
`tier-3`/`tier-4` 127, `tier-2,webview` 118, `tier-3,webview` 143,
`tier-3,headless` 191. Each total includes `#[ignore]`d tests, which a plain run
...
In-process EVM: no network, no `.env`. 222 tests across six files.
...
- **`test/Rub3CodeRegistry.t.sol`** (33) - the append-only properties of §2.9's code registry, asserted as behaviour rather than as comments, in six groups: publishing (the record kept whole, an unpublished hash reading as `Unknown`, the block stamped by the contract rather than supplied, the permanent `Published` event, every role carried through unchanged, and an empty offset table accepted as a real answer); append-only (republish and overwrite both reverting and leaving the record untouched, a deprecated hash not republishable either, and `test_audit_noRemovalOrRewriteSurfaceExists` scanning the deployed runtime bytecode for 10 removal, rewrite and un-deprecate signatures with its own positive control - a separate list from the shared 30, which is about tokens and says nothing about a registry); deprecation (the entire record compared either side of one, the reason logged, no repeat, no undo, and no reach into another record); ownership (non-owner writes reverting on both writers, the two-step transfer taking effect only on acceptance, and `renounceOwnership` reverting because ownership here is the right to *add*); offset tables (interned so identical tables are one entry, announced only on first use, rejected at four malformed shapes including the exact EIP-170 boundary either side, and readable as a bounded window - `offsetTableWindow` returns its bound out of a set four times larger, starts where it is asked to, and clamps rather than reverting past either end or at the largest `count` a caller can pass, which is what lets a purchase path read the bootstrap without paying for however many tables the owner key published); and record completeness (the zero hash, a missing source commit, each empty text field, and a reverted publish leaving neither an enumeration entry nor an interned table)
...
- `code_registry_answers_the_wrapper_over_a_real_chain_e2e` - publishes a release through `cast send` using the masked hash and immutable ranges out of `attest::CANONICAL` itself, then asserts the bounded `offsetTableWindow` read round-trips those ranges exactly, `consult_registry` returns the record with every field intact, code nobody published is `Unknown`, every `Role` and every reachable `Status` round-trips through the deployed contract, a hash nobody published decodes to no record at all, a `deprecate` reaches the wrapper as `Deprecated` with the record still whole, a bounded window read against a registry holding more tables than it asked for returns the bound and clamps when asked for more than exists, and a licence contract at the registry's place is never believed as the registry.
