...
```bash
# tier-3 (adds onchain-write + cooldown): 177 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3 --lib

# tier-3 + the headless (agent) front door: 243 lib tests
cargo test -p rub3-wrapper --no-default-features --features tier-3,headless --lib

# tier-3 + the webview (human) front door: 215 lib tests
...
it was written; §3.4's two seat-teardown store tests are two higher again in
every bundle that has a session model, which is every one of them except `tier-0`
and `tier-0,sdk`; on top of that, `tier-3,webview` carries the registry screen
...
- **`session_store::tests`** (requires `session` feature) - save/load round-trip, missing-session, `load_latest_session` picking the freshest valid session (`load_latest_session_for_wallet` narrows the same scan to one signer, covered from `activation::tests`), and the two halves of the expiry-agnostic scan the seat teardown path reads through (§3.4): `load_latest_session_any_expiry` finds a locally lapsed session that `load_latest_session` refuses, because a lapsed record still names the seat this machine took, and it still refuses a record whose signature does not verify, because one this machine did not write is not evidence of a seat it took
...
- `headless_release_seat_refuses_a_session_written_for_another_contract_e2e` (§3.4) - a §2.4 successor migration keeps the packed `APP_ID`, and session ids start at 1 on every deploy, so a predecessor's record can name a `(token, session)` pair that is live here and another instance's. The release is refused as `NoSeatHeld`, the other instance's session survives, and the record is still there to release against the contract it names
