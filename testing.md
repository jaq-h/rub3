...
For reference, `--lib` counts per bundle: `tier-0` 65, `tier-1` 95, `tier-2` 135,
`tier-3`/`tier-4` 175, `tier-2,webview` 136, `tier-3,webview` 212,
`tier-3,headless` 241, `tier-0,sdk` 77, `tier-3,sdk` 192. Every count on this
page is the number libtest reports as *running* - passed plus `#[ignore]`d, not
passed alone - so a plain run of `tier-3,webview` prints 206 passed and 6 ignored
against the 212 above. The five `packed` tests are in every bundle, which is why
...
- `headless_release_seat_frees_it_for_another_instance_e2e` (§3.4) - `release_headless` on a retiring machine frees its seat on-chain, leaves the other instance's session live, deletes the local record, answers a second call with `NoSeatHeld`, and lets the waiting machine start
...
- **`src/` unit tests** (18) - `deployments::tests` reads the *committed*
  ...
  "unknown", and `script_vars_names_every_variable_the_deploy_script_reads`,
  which pulls every `vm.env*` name out of `Deploy.s.sol` and asserts
  `SCRIPT_VARS` names it - a variable left out of that list is *inherited*
  rather than cleared, so a stale value in an operator's shell would become a
  deploy input nobody typed, and the omission is otherwise silent;
  `repo::tests` covers checkout resolution
