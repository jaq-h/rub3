...
2. Developer deploys an ERC-721 license contract on Base (`Rub3Access`)
...
│   │   ├── Rub3Factory.sol           # §2.3 - fee-stamping deploys + isDeployed, and its deployer helper
...
│   ├── script/
│   │   ├── Deploy.s.sol              # Deploy a Rub3Access to any EVM chain, directly or via FACTORY
...
- **Contracts** - `Rub3Access`, the one licence model: ERC-721 + Enumerable, bought once on either payment rail, valid for as long as it is held, with tier-3 `activate` + per-seat cooldown and §3.4 concurrent seats. 295 forge tests
...
- **Deploy scripts** - `forge script` deploys the licence contract to any EVM chain from env vars, directly or through a factory; `DeployFactory.s.sol` deploys the factory itself
- **`rub3 pack` and `rub3 deploy` (§2.5)** - one command builds the wrapper, the application and the configuration into a single distributable, with the canonical factory read out of `contracts/deployments.json` and compiled in beside the contract and chain id; another deploys the licence contract through that factory by driving the existing forge script. A `null` manifest entry is a hard error at both, so both refuse for every chain until launch. `pack` and `deploy` are the whole of the Phase 2 CLI: `rub3 fetch` and `rub3 register` are specified under the sections whose on-chain surface they call, §3.1 and §3.2, and neither is built - there is no content-addressed distribution to fetch from, and the discovery registry is built but deployed nowhere, so there is nothing to register with

**Not yet implemented (agent-first roadmap):** wrapper support for the `honorsContract` trust rule (the contract exposes and tests it; no shipped wrapper calls it, so a holder who claims onto a successor is not yet honored at launch), content-addressed distribution and the `rub3 fetch` subcommand that reads it (§3.1), the `rub3 register` subcommand and a deployed registry for it to write to (§3.2), concurrent-seat licensing, metered billing, marketplace. Human-surface polish (WalletConnect tabs, Preact refactor, Tauri plugin) is demoted behind the agent path; tier-4 device binding and binary encryption are deferred.
...
- **The token is the invariant; everything else is versioned.** Evolution only ever changes what is offered going forward (price, successor contracts, registry listings), never what was granted (held tokens and their validation). No proxies, no revocation surface - structurally, not by promise.
