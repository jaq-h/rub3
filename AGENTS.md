...
**`contracts/src/` is the publishing boundary, so a test-only contract must not live there.** The fingerprint script derives its deployable set from every artifact whose `compilationTarget` sits under the resolved source directory, which means anything added under `contracts/src/` is published as canonical rub3 code and then *required* to have a row in `attest::CANONICAL`. For a deliberately non-canonical fixture that inverts the check into a security regression with a green CI run. Fixtures go in `contracts/test/mocks/`; `NonCanonicalRub3Access.sol` is the worked example and its header explains the trap.
