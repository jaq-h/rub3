...
- A `purchase()` that is broadcast but whose receipt does not arrive inside the 30s budget exits **21**, not the retryable 14: the price may already have left the wallet, and an orchestrator following 14's "retry" advice would buy a second license while the first tx is still pending. `rpc::wait_for_receipt` now returns a typed `ReceiptWaitError` (`Timeout` vs `Transport`) so the two call sites classify it themselves; `activate()`'s own timeout stays 14, because a re-send either lands or hits the cooldown gate. The detail line carries `tx_hash` + `waited_secs` so the pending transaction can be resolved out of band
...
| 21 | purchase broadcast but not confirmed in time - terminal, `tx_hash=0x...` on stderr |
...
- `cargo test -p rub3-wrapper --no-default-features --features tier-3,headless --lib`: 104 pass
