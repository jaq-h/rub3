...
**Files** - `contracts/src/Rub3License.sol` (`SessionTerms`, `Seat`, `ActivationStatus`, `Selection`, the seat table, `activate`, `release`, `activationStatus`, `seatsInUse`, `seatAt`, `sessionSeat`, `lastActivationBlock`, `cooldownReady`), `Rub3Access.sol` + `Rub3Factory.sol` + `script/Deploy.s.sol` + `test/mocks/NonCanonicalRub3Access.sol` (the constructor change), `contracts/test/Rub3Seats.t.sol` (new), the four existing Solidity suites, `contracts/canonical-bytecode.json`; `crates/rub3-wrapper/src/rpc.rs` (the ABI mirror, `ActivationRecord`, `ActivationStatus`, `activation_from_receipt`, `activation_status`, `session_seat`, `occupied_seat`, `encode_release_calldata`, receipt logs), `session.rs` (`session_message`'s `chain_id` + `contract` fields, `Session.chain_id`, `session_expiry`, `verify_signature`, `verify_onchain`'s seat read and its `SeatNotHeld` refusal, the draft's activation record), `activation.rs` (`FleetExhausted`, `EXIT_FLEET_EXHAUSTED`, `release_headless`, `reclaim_seat`, `RecordRefusal`, `own_seat_record` - the one place a teardown path decides a record on disk names a seat this machine took, since everything downstream of it broadcasts `release`), `session_store.rs` (`delete_session`, `load_latest_session_for_contract`, `stored_record_count`), `webview.rs` + `assets/activation.html` (the exhaustion state, and the auto-detect gate below), `main.rs` (`--release-seat`), `attest.rs` (five signatures, three fingerprints); `crates/rub3-cli/src/deploy.rs` (`--seats`, `--session-ttl`).

**Tests** - 31 new in `contracts/test/Rub3Seats.t.sol`, 2 in `Rub3Invariants.t.sol`, 2 in `Rub3Access.t.sol`; 290 forge tests total, up from 255.
...
- Teardown authentication is proven against three forgeries in one arm: another key's record, an unsignable one, and a record this machine really wrote on another deploy with `contract` rewritten to name this one. Each is refused with the victim's seat still live and nothing signed
