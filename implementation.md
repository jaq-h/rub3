...
- `_afterClaim(tokenId, predecessorTokenId)` virtual hook; `Rub3Subscription` overrides it to carry the migrating holder's `expiresAt` *and* snapshotted `renewPrice` across, so migration is never a repricing event
...
- **Constructor probes the predecessor.** `_afterClaim` reads `expiresAt` / `renewPrice` off `predecessor`, and `predecessor` is immutable, so a `Rub3Subscription` deployed over a `Rub3Access` (or an EOA) would brick every holder's claim forever with redeployment the only remedy. The constructor rejects a non-zero predecessor that has no code or cannot answer `period()`, reverting with `IncompatiblePredecessor(address)` at deploy time
...
**Tests** - 83 forge tests, up from 33
- `test/Rub3Invariants.t.sol` (new): 43 tests - 18 on the hash set (constructor seeding, zero/duplicate rejection, empty set, older releases staying valid, revocation status + reason + events, revoked hashes not resurrectable, owner gating), 15 on succession (the three guarantees plus opt-in on both sides, one-claim-per-token, claim following the current holder, survival of renounced ownership, frozen subscription terms carried across, the trust rule and its survival of a successor repoint), 5 on the no-revocation audit, and 5 on mint ordering and predecessor typing (a `MintCallbackProbe` recipient reads the token from inside `onERC721Received` on both the purchase and the claim path and finds its terms already frozen; `IncompatiblePredecessor` on a `Rub3Access` predecessor and on a codeless one; a well-typed predecessor deploying and claiming end to end)
...
**Verification**
- `forge test`: 83 pass (26 Access + 14 Subscription + 43 Invariants), 0 failed
...
- The audit snippet documented in `contracts/contracts.md` run verbatim against a live deployment: all 18 forbidden selectors absent, positive control found
