...
- Full 11-bundle matrix green, clippy and fmt clean across it. `--lib` counts: `tier-0` 53, `tier-1` 83, `tier-2` 98, `tier-3`/`tier-4` 108, `tier-2,webview` 99, `tier-3,webview` 119, `tier-3,headless` 171, and the two new entries `tier-0,sdk` 65 and `tier-3,sdk` 125. Every bundle gains one test against those counts - the new `supervisor` one for the unconditional address publish, which is not feature-gated; the sibling scrub test it sits beside was tightened rather than added, and the three new `sdk` ones land only in the two bundles that compile that module. The `sdk` delta is +12 at tier-0 and +17 at tier-3, the difference being the five session-projection tests that need a session model to project
