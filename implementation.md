...
**Files** - `contracts/src/Rub3License.sol` (`SessionTerms`, `Seat`, `ActivationStatus`, `Selection`, the seat table, `activate`, `release`, `activationStatus`, `seatsInUse`, `seatAt`, `sessionSeat`, `lastActivationBlock`, `cooldownReady`), `Rub3Access.sol` + `Rub3Factory.sol` + `script/Deploy.s.sol` + `test/mocks/NonCanonicalRub3Access.sol` (the constructor change), `contracts/test/Rub3Seats.t.sol` (new), the four existing Solidity suites, `contracts/canonical-bytecode.json`; `crates/rub3-wrapper/src/rpc.rs` (the ABI mirror, `ActivationRecord`, `ActivationStatus`, `activation_from_receipt`, `activation_status`, `session_seat`, `occupied_seat`, `encode_release_calldata`, receipt logs), `session.rs` (`session_expiry`, the draft's activation record), `activation.rs` (`FleetExhausted`, `EXIT_FLEET_EXHAUSTED`, `release_headless`, `reclaim_seat`), `session_store.rs` (`delete_session`), `webview.rs` + `assets/activation.html` (the exhaustion state), `main.rs` (`--release-seat`), `attest.rs` (five signatures, three fingerprints); `crates/rub3-cli/src/deploy.rs` (`--seats`, `--session-ttl`).

**Tests** - 32 new in `contracts/test/Rub3Seats.t.sol`, 2 in `Rub3Invariants.t.sol`, 1 in `Rub3Access.t.sol`; 290 forge tests total, up from 258.
