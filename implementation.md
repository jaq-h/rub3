...
- **25 forbidden signatures × 3 deployed contracts** (`Rub3Access`, a successor `Rub3Access`, and the §2.3 `Rub3Factory`) as the list stands today: burn, admin transfer / seizure, pause, direct invalidation of a token, proxy/upgrade hooks, the removed `setWrapperHash` and any way to rewrite the set, forced migration, `setPredecessor` and `setPreviousFactory`, and the fee setters `setFeeBps` / `setTreasury`. Its history: 25 at §2.4, then 27 when §2.2 added the two per-token renewal-snapshot setters, then 29 when §2.3 added the fee setters, then 30 with `setPreviousFactory(address)` for the factory chain the canonical-predecessor rule walks - and back to 25 at §2.10, which took the five subscription-term setters (`setExpiresAt`, `setRenewPrice`, `setRenewPriceToken`, `setRenewPriceAmount`, `setPeriod`) and the subscription target with the model itself. A signature forbidding a setter for state no contract has is not a stronger guarantee, only a reader's dangling reference
...
- `attest::FORBIDDEN_SIGNATURES` mirrors the 25 signatures `test/Rub3Invariants.t.sol` asserts absent (30 when this section shipped; §2.10 removed the five subscription-term setters), searched over raw bytes rather than hex text so an odd-nibble coincidence cannot invent a finding
...
The behavioural tests sit underneath it rather than instead of it. `test_delisting_cannotTouchAHeldTokenOrALiveSession` buys a licence, activates a session, then pulls every discovery lever at once - the owner delists, the registry suspends, the payment token stops being recognised - and measures `ownerOf`, `honorsContract`, `activeSessionId`, a fresh activation past the cooldown, a fresh purchase and a transfer. `test_registryWrites_leaveTheLicenseContractUntouched` snapshots nine pieces of licence state across every registry write.
