...
**Tests** - 62 new in `test/Rub3Registry.t.sol`, `forge test` 286 passing (up from 224). `scripts/canonical-bytecode-hashes.sh check` and `scripts/check-deployments.sh` green. The anvil-gated `code_registry_e2e` passes against a real chain with the fifth role over the wire.
