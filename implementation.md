...
- A refusal shows its finding verbatim; a failed read shows only the kind of failure. The transport error is built from the request and so carries the packed `RPC_URL`, which can hold a provider API key, and the screen is telling the buyer to forward what it shows. The full error still goes to stderr, where the audience is whoever launched the wrapper
