...
- `rub3-wrapper --version` now answers which licence the binary gates on, on which chain, through which factory, at which tier and through which front doors. A distributable is run by somebody who did not build it, and all of that is compiled in, so it costs no network call
...
- The signer is not a flag. Everything after `--` goes straight to `forge script`, so `--private-key`, `--account`, `--ledger` and `--verify` work as `contracts/contracts.md` documents them, and no key material passes through an argument this CLI parses
...
**Tests** - 17 unit and 17 end-to-end in the CLI, 4 more in `packed`, plus 5 `#[ignore]`d build-gate tests
