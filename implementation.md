...
>
> - **One licence model.** rub3 sells a one-time purchase and nothing else. The time-bounded `Rub3Subscription` was built in Phase 1 and removed in §2.10, before any deploy.
>
> Phase 1 sections below are the build record of what exists today. They are corrected in place when a later phase supersedes a fact, with the pointer to the section that superseded it; §2.4 carries the removal record for everything the ownership invariants took out, and §2.10 the one for the second licence model.
...
- Mutable + owner-gated: `price` (`setPrice`). **Superseded by §2.4:** the single rotatable `wrapperHash` is gone, and `setWrapperHash(bytes32)` is now one of the forbidden selectors asserted absent from the runtime bytecode of all three audited targets. Wrapper hashes live in an append-only set with on-chain revocation reasons instead, so rebuilding the wrapper adds a hash rather than replacing one. See §2.4 for the removal record
...
**`Rub3Subscription.sol`** - concrete, time-bounded. **Removed by §2.10, and it is gone rather than deprecated:** immutable `period`, `expiresAt`, `renew`, `isValid` and the `Renewed` event no longer exist anywhere in the tree, and neither does the contract. It was never deployed to any network, so nothing was stranded. See §2.10 for the removal record; everything below about a second licence model is a record of what once was.
...
**`script/Deploy.s.sol`** - forge script that deploys either contract from env vars (one contract since §2.10):
- `CONTRACT_TYPE`, `TOKEN_NAME`, `TOKEN_SYMBOL`, `IDENTITY_MODEL`, `WRAPPER_HASH`, `PRICE` required; `SUPPLY_CAP`, `OWNER`, `COOLDOWN_BLOCKS` (default 1800 ≈ 1hr on Base), `PERIOD` optional. Superseded: `WRAPPER_HASH` is now optional and is the single-hash shorthand for `WRAPPER_HASHES` (§2.4); later phases added `PRICE_TOKEN` / `PRICE_AMOUNT` (§2.2), `PREDECESSOR` (§2.4) and `FACTORY` (§2.3); and §2.10 removed `CONTRACT_TYPE` and `PERIOD` outright with the second licence model, so the script deploys a `Rub3Access` and nothing else. `contracts/contracts.md` -> "Environment variable reference" is the current reference
...
- `session.rs`: 2 new preimage tests - differs by identity (access → account), differs by user_id alone; 1 new verify test - tampered identity fails `verify_local` with `AddressMismatch`; all existing tests updated to the new 10-arg `session_message()` signature (12-arg since §3.4)
...
- `rpc.rs` additions: `cooldown_ready` → `(is_ready, blocks_remaining)`, `last_activation_block`, `cooldown_blocks`, `active_session_id` (post-tx revocation read), `encode_activate_calldata` (pure, `SolCall::abi_encode`), `get_tx_receipt` → `TxReceipt { status, block_number, block_hash, to }`, `get_block_number`. **Since §3.4 neither `cooldown_ready` nor `active_session_id` exists**: `activation_status` answers both refusals in one read (the contract keeps its `cooldownReady` view), and `TxReceipt` also carries `logs`, which is where the session id and the seat now come from
...
- Connect handler branches: zero contract → legacy `show_activate`. Non-zero + `cooldown` → `tokens_of_owner` → `proceed_after_token_selected` → `cooldown_ready` + `encode_activate_calldata` → `onShowCooldown` (**since §3.4 that read is `activation_status`**, which tells a cooldown from a full fleet)
- ActivateTxSent handler: spawns a background polling thread (10 × 3s; 30s total timeout) calling `get_tx_receipt`; on confirmation asserts `receipt.to == contract` and `status == true`, reads `activeSessionId`, mints a `new_nonce()`, computes `expires_at` from `SESSION_TTL_SECS`, builds the session message, and emits `onTxConfirmed`. Since §3.4 the session id and the seat come from the `Activated` log in that transaction's own receipt rather than from a follow-up state read, and `expires_at` is clamped to the seat's on-chain expiry
...
- `ensure_headless(&dyn Signer, &HeadlessContext) -> (Session, HeadlessOutcome)`: cached-session fast path → chain-id guard → `tokens_of_owner` → (empty → `purchase()`) → `cooldown_ready` (**`activation_status` since §3.4**) → `activate()` → `wait_for_receipt` → `draft_from_activation` → `personal_sign` → `verify_local` → `save_session`
...
- `purchaseAuthorizationNonce(recipient, salt) = keccak256(tag, address(this), recipient, salt)`, and `Rub3Subscription.renewAuthorizationNonce(tokenId, salt)` with a different tag. A changed recipient (or token id) derives a different nonce, which yields a digest the buyer never signed, and the token rejects it. **§2.10 removed the renewal half:** buying is the only thing the contracts derive a nonce for now, so `rub3.PurchaseAuthorization.v1` is the only tag left and it stays in the preimage rather than being dropped as a constant that discriminates nothing
- §2.2 shipped two distinct domain tags (`rub3.PurchaseAuthorization.v1` / `rub3.RenewAuthorization.v1`) so that a purchase authorization could never be spent as a renewal, or the reverse. That is why the surviving tag stays: a preimage already signed cannot be re-tagged afterwards, so the discipline outlives the second tag §2.10 removed
...
Duplicated mint logic is how the two paths would drift. `Rub3Access` gained a private `_mintPurchased(to, payer)` and `Rub3Subscription` a private `_mintSubscription(to, payer)`; `purchase` and `purchaseWithAuthorization` differ only in how payment is taken and who is named as `payer` (`msg.sender` on the ETH rail, `auth.from` on the other, where `msg.sender` may be a facilitator who paid nothing but gas). `renew` and `renewWithAuthorization` share `_extend` the same way. Since §2.10 there is one concrete contract and `_mintPurchased` is the one mint the two rails share. `_resolveAuthorizedRecipient` is deliberately *not* `_resolveRecipient`: a zero recipient means the buyer who signed, never the submitter.
...
**How the token-denominated renewal price relates to `renewPrice`** - `Rub3Subscription.sol` *(removed by §2.10 along with the contract; recorded because the reasoning about independently listed rails outlived it)*
...
`price`, `priceToken`, and `priceAmount` are one `SaleTerms` struct rather than three loose arguments. Passing them loosely put `Rub3Subscription` one stack slot over solc's limit in the constructor ABI decoder; grouping also names the concept and keeps the two rails visibly parallel. `Rub3Access` stays at 10 constructor arguments and `Rub3Subscription` at 11. The struct outlives the contract that forced it: §2.10 kept it, because ungrouping it would have moved every remaining fingerprint to say the same thing less clearly. `forge create` takes the struct as a parenthesised tuple, `"(<price>,<priceToken>,<priceAmount>)"`. Threaded through `script/Deploy.s.sol` (new optional `PRICE_TOKEN` / `PRICE_AMOUNT`), all three Foundry fixtures, and both wrapper e2e tests.
...
- The audit now runs against **four** targets rather than three: `Rub3Factory` joins the two licence contracts and the successor, because the factory is where the terms are chosen and a setter there would unfreeze them just as surely (three since §2.10, which took the subscription target with it)
...
- `contracts/canonical-bytecode.json` regenerated with `scripts/canonical-bytecode-hashes.sh update`; the blocking `check` gate passes. Five contracts are fingerprinted at this point, up from two (§2.9 and §3.2 later take it to seven, and §2.10 back to five - a different five)
...
**Deliberately not done**
- **No fee anywhere but the payment functions**, `purchase()` and `renew()` at the time and `purchase()` alone since §2.10. Deploys are free (the factory charges nothing and the developer pays only gas), and the CLI, SDK, and wrapper are untouched. There is no token
...
- Revoking a *binary hash* is structurally unable to reach *token validity*: `ownerOf`, `honorsContract` and `activate` never read the set (`isValid` too, while the second model existed)
...
- `_afterClaim(tokenId, predecessorTokenId)` virtual hook; `Rub3Subscription` overrides it to carry the migrating holder's `expiresAt` *and* snapshotted `renewPrice` across. Reads go through `IRub3Predecessor`, the view-only slice a successor is allowed to touch, so migration can never disturb the old contract. **§2.10 removed the hook with its only implementor**, and trimmed `IRub3Predecessor` back to `ownerOf` and `successor`: a claim carries nothing, because a licence has no ongoing terms to carry
...
**Per-token renewal snapshot** - `Rub3Subscription.sol` *(removed by §2.10 with the contract)*
...
- **30 forbidden signatures × 3 deployed contracts** (`Rub3Access`, a successor `Rub3Access`, and the §2.3 `Rub3Factory`) as the list stands today: burn, admin transfer / seizure, pause, direct invalidation of a token, proxy/upgrade hooks, the removed `setWrapperHash` and any way to rewrite the set, forced migration, `setPredecessor` and `setPreviousFactory`, the fee setters `setFeeBps` / `setTreasury`, and the five seat signatures §3.4 added. Its history: 25 at §2.4, then 27 when §2.2 added the two per-token renewal-snapshot setters, then 29 when §2.3 added the fee setters, then 30 with `setPreviousFactory(address)` for the factory chain the canonical-predecessor rule walks, back to 25 at §2.10, which took the five subscription-term setters (`setExpiresAt`, `setRenewPrice`, `setRenewPriceToken`, `setRenewPriceAmount`, `setPeriod`) and the subscription target with the model itself - and 30 again at §3.4, which added the three that would unfreeze a seat count or a session TTL (`setSeatsPerToken`, `setMaxConcurrentSessions`, `setSessionTtl`) and the two that would let anybody but a holder end a live session (`revokeSeat`, `clearSeats`). A signature forbidding a setter for state no contract has is not a stronger guarantee, only a reader's dangling reference
...
- Behavioural companions: the contract owner cannot `transferFrom` / `safeTransferFrom` / `approve` a token it does not hold; and an "owner does its worst" test runs every owner-only function that exists (max out the price, add a hash, revoke every hash, repoint the successor, drain the balance, hand ownership to an attacker, who repeats it and then renounces) and asserts `ownerOf`, `activate` and transfer all survive

**Tests** - 90 forge tests, up from 33

- `test/Rub3Invariants.t.sol` (new): 50 tests in four groups - 18 on the hash set, 16 on succession, 11 on mint ordering and predecessor typing, 5 on the no-revocation audit (39 since §2.10; `testing.md` holds the current split)
...
rub3 deploy --name "My App License" --symbol MAL \
...
- `attest::FORBIDDEN_SIGNATURES` mirrors the 30 signatures `test/Rub3Invariants.t.sol` asserts absent (30 when this section shipped, 25 after §2.10 removed the five subscription-term setters, 30 again after §3.4 added five seat signatures), searched over raw bytes rather than hex text so an odd-nibble coincidence cannot invent a finding
...
- The prohibitions are absent from the bytecode rather than merely unused, and `test_audit_noRemovalOrRewriteSurfaceExists` asserts it against the deployed runtime code with the same scanner `Rub3Invariants.t.sol` uses, plus its own positive control. **This is a separate 10-name list, not the shared one** (30 names when this section shipped, 25 since §2.10). That list is about tokens - burn, seize, pause, upgrade - and says nothing about a registry; what would undo *this* contract's guarantee is a remove, a rewrite, or a status moving backwards. No count in the shared list moved, because no external function was added to a licence contract or to `Rub3Factory`
...
Computing a masked hash needs the immutable ranges; finding the record needs the hash. The registry interns the *distinct* tables its releases use and returns them in one call, so an agent fetches the short candidate list first and computes a hash under each - up to a cap, since each surviving candidate costs a round trip on a path that spends money. Today's canonical set spans four of them: one each for `Rub3Access`, `Rub3Factory` and `Rub3Registry`, plus the empty one `Rub3AccessDeployer` and `Rub3CodeRegistry` itself share (five across seven contracts until §2.10 removed two). `publish` bounds what may go in one: each range is exactly one 32-byte word, ranges are sorted and disjoint, and none may fall outside EIP-170's 24,576-byte cap - which together bound a table to 768 ranges without inventing a limit of its own.
...
### 2.10 - One licence model: removing `Rub3Subscription` `[complete]`

rub3 sold two licence models. It now sells one: `Rub3Access`, bought once and valid for as long as the token is held. The time-bounded `Rub3Subscription` is **gone from the tree** - not deprecated, not feature-gated, not commented out - along with its deployer helper, the factory's arm for it, its fingerprint rows, and every document that described a choice between two models. Nothing was ever deployed to any public network (`contracts/deployments.json` is `null` on both chains), so no contract was stranded and no migration was owed.

**Why, and why it had to actually happen.** A registry review raised that the §3.2 agent card cannot distinguish a one-time purchase from a subscription that expires, so an automated buyer could pay for access that turns out to be time-bounded. The proposed fix was to teach the card a licence-model flag plus period and supply. That fix was declined for one reason: the card is immutable once deployed, and teaching it to describe a distinction the project was about to delete would have frozen the wrong thing permanently. **That reasoning is only sound if the second model actually goes away**, and it is the whole justification for a finding declined on a contract that cannot be changed afterwards. This section is that removal. Had it not landed, the gap would have gone live and been unfixable.

**Contracts**

- `contracts/src/Rub3Subscription.sol` deleted. With it: `period`, `expiresAt`, `renewPrice` / `renewPriceToken` / `renewPriceAmount`, `renew` / `renewWithAuthorization`, `renewAuthorizationNonce`, `isValid`, and the `Renewed` event. **`isValid` was a subscription-only function**, so validity is now `ownerOf` alone, with `honorsContract` the read that spans a succession - every doc naming `isValid` as an invariant target was corrected to name those instead
- `Rub3License` loses the machinery that existed only for the second model: `IRub3Predecessor` is back to `ownerOf` and `successor`, the `_RENEW_AUTHORIZATION` domain tag is gone, and so is `_afterClaim`, the virtual hook whose only implementor carried subscription terms across a claim. `_PURCHASE_AUTHORIZATION` stays in the nonce preimage rather than being dropped as a constant that now discriminates nothing: an untagged nonce is one a second derivation added later could reproduce verbatim, and the preimage of an authorization already signed cannot be changed afterwards
- `Rub3Access`'s constructor loses its model probe. It rejected a predecessor that answered `period()`, which was how cross-model succession was made impossible in both directions. With one model there is nothing to discriminate: `Rub3License`'s own probe (code present, answers `successor()`) is the whole check, and it is what the immutable pointer actually needs
- `Rub3Factory` loses `Rub3SubscriptionDeployer`, the `subscriptionDeployer` immutable, and `deploySubscription`. **`LicenseDeployed` loses its `uint8 model` field** rather than keeping one that is always `0`; a field with one possible value is a reader's dangling reference, not information. The `Rub3AccessDeployer` split stays: `Rub3Access`'s creation code alone is 16.1 KB against a 24,576-byte runtime limit, so a factory that could `new` it directly would have almost nothing left for itself. The factory's own initcode falls to 21.8 KB from the 42.0 KB of §2.3, well clear of EIP-3860's 49,152
- `script/Deploy.s.sol` loses `CONTRACT_TYPE`, `PERIOD`, and the `_eq` / `_capitalize` helpers that existed to branch on the type string. It deploys a `Rub3Access`, directly or through `FACTORY`
- **Forbidden-selector list 30 -> 25.** `setExpiresAt(uint256,uint256)`, `setRenewPrice(uint256,uint256)`, `setRenewPriceToken(uint256,address)`, `setRenewPriceAmount(uint256,uint256)` and `setPeriod(uint256)` went with the state they would have rewritten, and the audit runs against three targets rather than four. A signature forbidding a setter for state no contract has is not a stronger guarantee; it is a name a reader has to look up and fail to find. Swept in the same pass across all five places `AGENTS.md` names: the `string[N]` array, the loop in `contracts/contracts.md`, the bytecode table in `architecture.md`, `attest::FORBIDDEN_SIGNATURES`, and the count in §2.4 above. The wrapper's mirror test is what keeps two of those honest

**Fingerprints, regenerated in the same commit** - `contracts/canonical-bytecode.json`, `attest::CANONICAL`

Seven fingerprinted contracts become five, and the interesting part is which of the survivors moved:

| Contract | Fingerprint |
|---|---|
| `Rub3Access` | **unchanged** |
| `Rub3AccessDeployer` | moved |
| `Rub3Factory` | moved, 12 immutable ranges -> 10 |
| `Rub3CodeRegistry` | unchanged |
| `Rub3Registry` | unchanged |

`Rub3Access`'s runtime code is byte-identical, which is the strongest available statement that **nothing about the one-time purchase model changed behaviourally**: everything removed from it was constructor code (the model probe, which lives in initcode) or code the optimiser never emitted (an empty virtual hook, an unread constant). The deployer moved because it *carries* that constructor code; the factory moved because it lost a deploy function and an immutable. Nothing is deployed, so the two moved rows were corrected in place rather than accumulated - the pre-deploy case `attest::CANONICAL`'s doc comment describes - and `RELEASE` moved to name this release. The canonical set now spans four distinct offset tables rather than five, well inside the wrapper's candidate budget of sixteen.

**Wrapper and CLI**

- `attest.rs`: the two removed rows, the shortened forbidden list, and every count that followed from them. `Role::Deployer` now names one helper. Nothing on the launch or purchase path changed shape
- `rub3 deploy` loses `--type` and `--period`, and `CONTRACT_TYPE` / `PERIOD` leave `SCRIPT_VARS`, the list of variables the command scrubs from the child's environment. A flag naming which of one contract to deploy is a choice with one option; the summary no longer prints a `type:` line

**Tests: 293 -> 255, with the shared machinery rehomed rather than dropped**

The distinction that mattered here is between a test that covered the subscription and one that covered something shared *through* the subscription. The first kind was deleted; the second was moved onto `Rub3Access`.

- `test/Rub3Subscription.t.sol` deleted whole (15). Every claim in it was about expiry, renewal or the per-token renewal snapshot; `_payEth`'s exactness, metadata and the supply cap are covered in `test/Rub3Access.t.sol` already
- **Rehomed.** `test_mintOrdering_claimStateExistsBeforeRecipientCallback` used a subscription fixture to prove per-token state is written before `_safeMint` hands control to a contract recipient; it now runs on `Rub3Access`, where `claimFromPredecessor` is the one mint path that writes any. `test_subscription_recipientCallbackSeesFrozenTermsOnTheTokenRail` became `test_recipientCallbackSeesAPaidForToken`, asserting the money is in the contract and the token already the recipient's by the time the callback runs. `test_eth_renewalIsChargedToo` and its stablecoin twin proved a *second* payment accrues a second fee, which renewal happened to be the second payment; they now buy twice and assert the same thing. `test_delisting_cannotTouchAHeldTokenOrALiveSession`, §3.2's acceptance test, pulls every discovery lever on a `Rub3Access` and measures `ownerOf`, `honorsContract`, the live session, a fresh activation, a fresh purchase and a transfer
- **Deleted as duplicates of an access-side test that already existed**: the subscription arms of the revoked-hash, owner-does-its-worst, both-rails-mint-identically and predecessor-probe groups, and the `wrongIntent` pair that proved the two domain tags were disjoint - there is one tag now, and `test_wrongContract_authorizationDoesNotTransplant` still pins the contract binding
- `Rub3Factory.t.sol`'s `sub` fixture became a second `Rub3Access` deployed through the same factory, so the record-keeping tests (`isDeployed`, ordered enumeration, owner defaults) still run against more than one row
- No test was left skipped, ignored or orphaned, and nothing was disabled to make the suite pass

**Verification**

- `forge test`: 255 pass (28 Access + 39 Invariants + 32 TokenPurchase + 52 Factory + 35 CodeRegistry + 69 Registry), 0 failed
- `scripts/canonical-bytecode-hashes.sh check` green against the regenerated manifest, in the same commit as the removal, so no commit in this branch's history has the manifest and `attest::CANONICAL` out of step
- Full wrapper matrix, the SDK, the CLI, `forge fmt --check`, `cargo fmt --check`, `cargo clippy -- -D warnings`, and the anvil-gated suites

**Deliberately not done here**

- **`Rub3Metered` (§4.1) stays.** It is a per-launch micropayment model and it is `[not started]`. It is not a subscription: it bounds what a *launch* costs and there is no path in it that makes an issued token expire, so removing it would have been a different product decision rather than this one
- **`SaleTerms`, `FeeTerms` and `IdentityTerms` stay grouped.** All three exist because `Rub3Subscription`'s constructor kept hitting solc's stack limit in the ABI decoder (§2.2, §2.3), and the contract that forced them is gone. Ungrouping them would move every remaining fingerprint, change `Rub3LicenseParams`, and reach the CLI and the deploy script, to say the same thing less clearly - a redesign, and this was a removal
- **`Rub3License` stays abstract with one subclass.** The split still says something: the base is the machinery every rub3 licence contract has, `Rub3Access` is what a licence *is*. Collapsing them would move `Rub3Access`'s fingerprint - the one thing this section proved did not move - and §3.4's concurrent seats lands on the base
...
The behavioural tests sit underneath it rather than instead of it. `test_delisting_cannotTouchAHeldTokenOrALiveSession` buys a licence, activates a session, then pulls every discovery lever at once - the owner delists, the registry suspends, the payment token stops being recognised - and measures `ownerOf`, `honorsContract`, `sessionSeat`, a fresh activation past the cooldown, a fresh purchase and a transfer. `test_registryWrites_leaveTheLicenseContractUntouched` snapshots nine pieces of licence state across every registry write.
...
### 3.4 - Concurrent seats `[complete]`

Fleet licensing - the tier the agent economy actually wants. A licence stops being one seat and becomes K: one licence NFT admits `seatsPerToken` concurrent sessions, and buying another token is how a fleet scales past that.

Tier 3's single `activeSessionId` is now a per-token seat table. `activate()` takes the first seat that is available *and* off its own cooldown; `release(tokenId, sessionId)` or a TTL lapse frees one.

**Where K is set, and the two readings that were rejected.** `seatsPerToken` is `immutable` and per contract. A developer selling two seat counts deploys two contracts, which is what "buy another token to scale" already described. Both alternatives were costed and turned down:
- **Per token, snapshotted at mint.** Invariant-safe in the same way §2.4's `renewPrice` snapshot was, but it puts a per-token SSTORE back on the purchase path and opens a race `price` does not have: `_payEth` demands the exact price, so a price moved between a buyer's read and their transaction *reverts*, while moved seats would silently hand them fewer for the same money. Closing that needs `purchase(recipient, expectedSeats)` on both rails, reaching the wrapper's calldata builder, the CLI and the ABI mirror
- **Owner-adjustable after mint.** Lowering is a revocation surface and is ruled out by §2.4 outright. *Raise-only* survives the letter of it - a monotone increase takes nothing away - and is still rejected, for two reasons this repository had already settled: `cooldownBlocks` is immutable specifically so the owner cannot defeat rate limiting, and K is a straight multiplier on that same rate; and it would make the pre-purchase audit non-final, when §2.6's attestation deliberately reports only what is frozen. What it buys - a developer being generous later - the successor pattern already provides

**The cooldown moves from per token to per seat, and that is what preserves it rather than weakening it.** Left at the token level, filling K seats would take K cooldown windows, so at the documented 1800-block default a ten-instance fleet would take ten hours to start; seats would not function. Per seat, the property is exact: **a token lands at most `seatsPerToken` activations in any window of `cooldownBlocks` blocks** - `seatsPerToken` times the single-seat rate and not one more. The mechanism is one line of the design: `release()` and a TTL lapse free a seat's *occupancy* and never its `activatedAt` stamp. Clearing the stamp on release is the obvious wrong implementation and it is an unlimited churn bypass; `test_churnDefence_releaseDoesNotClearTheCooldownStamp` and its lapse counterpart exist to pin exactly that, and both were verified to go red against a mutation that clears it.

**TTL lapse: lazy, unswept, and paid for by the next activator.** A seat freed only by explicit `release()` strands whenever an instance dies, which is the normal case in a fleet - a licence would degrade one crash at a time to zero usable seats, which is a grant becoming nothing by accident. So every seat carries `expiresAt`, and `activate()`'s scan treats a lapsed seat as free. Nothing sweeps, no keeper exists, and no one pays to clean up after someone else: the scan is O(`seatsPerToken`) with one cold SLOAD per seat, which is why `MAX_SEATS` is 64 and why `activatedAt` and `expiresAt` share a storage slot. `sessionTtlSeconds` is a **timestamp** while the cooldown stays in blocks, and the two units are deliberate: a cooldown is a rate limit over chain progress, while a TTL is a wall-clock credential lifetime that has to line up with a session's RFC 3339 expiry, and converting blocks to seconds would need an assumed block time the contract has no business asserting.

**A single-seat licence is still exactly the tier-3 licence, and the contract says so.** With `seatsPerToken == 1` there is no fleet, and the one seat can only be the holder's, so `activate()` retakes it: one session at a time, opening a new one ends the old, refused by the cooldown and by nothing else. That is not a convenience - without it a sole holder whose session record went missing (a wiped cache, a fresh container) would be locked out of their own licence for a whole `sessionTtlSeconds`, which is a licence that stops working for the person who owns it. It costs no churn, because the retake still waits out the seat's own stamp. **Above one seat the rule stops**, and that is the point: there, an occupied seat you cannot account for is another instance's, and reporting it rather than evicting it is what `FleetExhausted` is for.

**The seats follow the token.** A transfer ends every session the previous holder had open: `_update` frees the occupancy of each live seat on a transfer and never on a mint, emitting `Released` for each, and leaves every seat's `activatedAt` stamp exactly where it was. Without it a resold fleet licence was unusable by its buyer for up to a whole `sessionTtlSeconds` - both seats live, `activate()` reverting `FleetExhausted`, and no record on any machine of theirs to `--release-seat` from - while the seller's instances kept launching on a licence they no longer held. Not a revocation surface: §2.4 protects what a holder still holds, the seller holds no token once the transfer lands, and the licence following the token is what "`ownerOf` is validity" already means. Nothing owner-callable or admin-callable was added; the audited signature list is unchanged. The stamps surviving is what keeps the churn defence whole across a sale: a token sold inside a seat's cooldown still owes that seat's blocks to its buyer, `activationStatus` reports it as a cooldown rather than a full fleet, and `test_churnDefence_transferDoesNotBuyAFasterChurn` measures the same K-per-window budget across two holders that the single-holder test measures for one. The seller's cached session fails its next sampled launch, since its seat is gone.

**The contract's TTL is authoritative over the wrapper's.** A session's `expires_at` is the *earlier* of the packed `--session-ttl` and the seat's own on-chain expiry, so packaging can shorten a session and never outlive the seat that admits it - the alternative is a token quietly running more sessions at once than it sold. On a build whose packed TTL is the shorter one, a cached session lapses locally while its seat is still live; the headless door hands that seat back before reporting a fleet it was itself filling, and only ever a seat its own store names and the chain agrees it still holds. One machine freeing another's seat would be one machine ending another's session, which §2.4 rules out even between two machines holding the same key.

**Where the seat bound is enforced, stated exactly.** At activation it is unconditional: `activate()` is the contract admitting the session at all, and a token past `seatsPerToken` live seats gets `FleetExhausted`. At launch it is re-read only on the launches `should_reverify` samples, about one in five, where `verify_onchain` asks `sessionSeat(tokenId, sessionId)` whether the seat is still that session's and fails the launch when it is not. Two consequences are stated rather than left to be inferred, because the security table used to claim more than the code does: a copied session file whose seat has since gone to somebody else keeps launching until its next sampled launch, and a copy taken while the original's seat is live shares that seat and is not detected at all, since two instances on one session id are one seat to the contract. Binding a session to the machine that took its seat is tier 4's job and tier 4 is deferred. The sampling rate was left where it was: raising it would trade a launch's offline tolerance and network cost for a bound it still could not make exact, and it is not this section's to change quietly.

**The record names its deploy, and the signature covers it.** `session_message` hashes `chain_id` and `contract` alongside the rest. Every guard on the teardown path asks "is this record one this build may act on" by comparing exactly those two fields against the packed ones, and the session directory is a user-writable directory: unsigned, they were the one part of a genuine record a tamperer could rewrite, and rewriting them repoints a `release(tokenId, sessionId)` at another deploy where the same ids name a stranger's live session. That is the revocation shape §2.4 rules out. The launch path holds the same line: `try_session_fast_path` takes the packed `chain_id` and `contract` and refuses a record whose signed pair differs, beside its wallet compare, and both launch scans narrow on that pair the way the teardown scan does, so a record a faucet-funded testnet activation wrote under the same `app_id` never launches the production build on the four launches in five that nothing samples. A §2.4 migration is holder-initiated and writes a fresh record for the successor, so no supported flow produces a record this guard turns away; the launch path reads no `honorsContract`, deliberately. The format change costs nothing today because nothing is deployed and no fielded binary carries the old preimage.

**A refused record is named on stderr, and the exit code does not move.** `--release-seat` answering `released=false` used to mean either "this machine holds nothing" or "a record was found and refused", and the second is the one that matters: a session file a crash truncated leaves a seat held for the rest of `sessionTtlSeconds` with nothing on this machine able to hand it back. `ReleaseOutcome::NoSeatHeld` now carries the refusal, `main.rs` prints one warning and adds `rejected=<reason>` to the detail line, and the outcome stays a success - so an orchestrator branching on the code sees no change and one reading the detail line can branch on the reason.

**Auto-detect is offered at one seat and nowhere above it.** §5.1a's watch resolves an `activate()` from the chain alone: the block `lastActivationBlock` names, filtered to the contract and the token. At one seat that block holds one activation of that token and it is the one the person on the screen just sent. Above one seat it is the ordinary case for a fleet instance to activate the same token in the same block, and every instance signs with the holder's key, so no topic separates the two logs - the window would sign a session over another instance's `sessionId` and `seatIndex`, which is two live sessions on one seat and exactly the over-admission seats exist to prevent, reached in silence. So the cooldown screen reads the seat count when it opens and above one seat offers the Manual tab alone, where a pasted transaction hash is unambiguously the user's. At one seat the screen is unchanged, which is the same constraint that produced the sole-holder retake.

**§3.4's own text described a world that had gone.** It was written against two licence models and specified K as a mapping "set at purchase tier / deploy"; since §2.10 there is one model, one `price`, and no tier concept in the contracts for "purchase tier" to refer to. This section is the corrected statement.

**Files** - `contracts/src/Rub3License.sol` (`SessionTerms`, `Seat`, `ActivationStatus`, `Selection`, the seat table, `activate`, `release`, `activationStatus`, `seatsInUse`, `seatAt`, `sessionSeat`, `lastActivationBlock`, `cooldownReady`, the `_update` override that ends the previous holder's sessions on a transfer), `Rub3Access.sol` + `Rub3Factory.sol` + `script/Deploy.s.sol` + `test/mocks/NonCanonicalRub3Access.sol` (the constructor change), `contracts/test/Rub3Seats.t.sol` (new), the four existing Solidity suites, `contracts/canonical-bytecode.json`; `crates/rub3-wrapper/src/rpc.rs` (the ABI mirror, `ActivationRecord`, `ActivationStatus`, `activation_from_receipt`, `activation_status`, `session_seat`, `occupied_seat`, `encode_release_calldata`, receipt logs), `session.rs` (`session_message`'s `chain_id` + `contract` fields, `Session.chain_id`, `session_expiry`, `verify_signature`, `verify_onchain`'s seat read and its `SeatNotHeld` refusal, the draft's activation record), `activation.rs` (`FleetExhausted`, `EXIT_FLEET_EXHAUSTED`, `release_headless`, `reclaim_seat`, `RecordRefusal`, `own_seat_record` - the one place a teardown path decides a record on disk names a seat this machine took, since everything downstream of it broadcasts `release`), `session_store.rs` (`delete_session`, `load_latest_session_for_deploy`, `is_for_deploy` and the deploy-narrowed launch scans, `stored_record_count`), `webview.rs` + `assets/activation.html` (the exhaustion state, and the auto-detect gate below), `main.rs` (`--release-seat`), `attest.rs` (five signatures, three fingerprints); `crates/rub3-cli/src/deploy.rs` (`--seats`, `--session-ttl`).

**Tests** - 36 new in `contracts/test/Rub3Seats.t.sol`, 2 in `Rub3Invariants.t.sol`, 2 in `Rub3Access.t.sol`; 295 forge tests total, up from 255.
- The churn defence is measured, not argued. `test_churnDefence_atMostKActivationsPerCooldownWindow` activates *and releases* greedily at every block of a window, so occupancy can never be what stops the next attempt and the count is the churn budget alone: exactly K, then nothing for the rest of the window, then exactly K again
- Four mutations were applied one at a time and each was caught: clearing the stamp on `release`, treating a lapsed seat as never activated, making an occupied seat retakeable at any K, and reporting exhaustion as a cooldown. The first two make the greedy loop run out of gas, which is the bypass made visible
- `test_audit_ownerCannotReachASeat` runs every seat-shaped call an owner might reach for against a live fleet and asserts each hits the missing fallback, then that `release` refuses the contract owner - the behavioural half of the five new forbidden signatures
- Four wrapper e2e arms against anvil: a three-instance fleet on one token with the fourth refused, `--release-seat` freeing a seat for a waiting instance while leaving the other instance's session untouched, a released seat still costing its own cooldown, and a session's expiry equal to its seat's rather than to the seven-day packed TTL
- The launch-time bound is asserted at the rate it actually holds: `headless_a_sampled_launch_refuses_a_session_whose_seat_is_gone_e2e` copies a live record to a machine that never activated, shows it launching while the seat is live, hands the seat to somebody else, and then launches in a loop until a sampled launch refuses it - both halves, because a test asserting only the refusal would be evidence for a stronger claim than the code makes
- Teardown authentication is proven against four forgeries in one arm: another key's record, an unsignable one, a record this machine really wrote on another deploy with `contract` rewritten to name this one, and one it really wrote for this contract's address on another chain, which verifies exactly as signed and is refused on its signed `chain_id` alone. Each is refused with the victim's seat still live and nothing signed
- The teardown scan narrows on the deploy and the key, not the contract alone: two `session_store` tests pin that a record signed for the same address on another chain is never chosen, and that a newer record under another agent's key on the same machine cannot hide this key's own releasable one
- The sale is pinned from both sides in `Rub3Seats.t.sol`: a two-seat token sold with both seats live is the buyer's to fill, with both of the seller's sessions ended and reported; a token sold inside a seat's cooldown still owes the buyer that seat's blocks, with the stamps read back unchanged; the K-per-window budget is measured across the sale; a mint ends nothing; and only the sessions that were live are reported
- Full 11-bundle matrix green; `--lib` counts in `testing.md`
...
- Second billing model, and the only one still planned: the launch gate requires a micropayment - per launch, per session-hour, or per N launches - settled in USDC (EIP-3009 authorizations batched/settled on-chain). It is not the model §2.10 removed: it bounds what a *launch* costs and never makes an issued token expire.
...
- Hosted relay that submits EIP-3009 purchase and meter authorizations and fronts gas for buyers holding only stablecoins.
...
│   │   ├── Rub3Access.sol            # The license: one-time purchase, valid while held
│   │   ├── Rub3Factory.sol           # §2.3 - fee-stamping deploys + isDeployed
│   │   ├── Rub3CodeRegistry.sol      # §2.9 - append-only bytecode version authority
│   │   └── Rub3Registry.sol          # §3.2 - discovery: which apps exist and are listable
│   ├── test/
