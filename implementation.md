...
- `Rub3Factory` loses `Rub3SubscriptionDeployer`, the `subscriptionDeployer` immutable, and `deploySubscription`. **`LicenseDeployed` loses its `uint8 model` field** rather than keeping one that is always `0`; a field with one possible value is a reader's dangling reference, not information. The `Rub3AccessDeployer` split stays: `Rub3Access`'s creation code alone is 16.7 KB against a 24,576-byte runtime limit, so a factory that could `new` it directly would have almost nothing left for itself
