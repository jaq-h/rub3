...
**The packed endpoint is redacted at the RPC layer, not at a screen.** `RPC_URL` can carry a provider API key in its userinfo, its path or its query, and alloy builds its error messages from the request, so the key rides inside the `RpcError` value. `RpcError::transport` and `RpcError::contract` rewrite every URL in a message to `scheme://host[:port]` at construction, and every construction in `rpc.rs` built from a network or contract error now goes through them. Sanitizing at construction rather than at a display helper is the point: the same string reaches the window's error box, the agent door's printed `HeadlessError::Rpc`, and `show_purchase`'s stderr line, and only the constructor is upstream of all three. Both constructors were needed, not just the transport one - alloy reports an unreachable node during an `eth_call` as a *contract* error, which is how the window's "ownership check failed" box, the first error surface any buyer meets, was the most-travelled leak. The host and port stay: an operator chasing a dead endpoint needs to know which one failed, and neither is the secret.
...
**Tests** - 6 unit tests in `webview`, compiled only under a bundle where a purchase can be made from the window
...
- In `rpc`: `a_key_in_the_endpoint_url_never_survives_into_the_error` covers a key in a path segment, in a query parameter and as userinfo, through both constructors, and asserts the host and the failure text survive; `redaction_keeps_the_origin_and_the_words_around_it` pins the rewrite itself, including the port and the prose either side; `an_unreachable_node_leaks_no_key_through_the_ownership_check` drives `tokens_of_owner` against a dead port so the contract-error classification is observed rather than assumed
- Full 9-bundle matrix green. `--lib` counts: `tier-0` 49, `tier-1` 79, `tier-2` 94, `tier-3`/`tier-4` 104, `tier-2,webview` 94, `tier-3,webview` 111, `tier-3,headless` 165. Every bundle gains the three `rpc` redaction tests, since the sanitizing constructors are not feature-gated; `tier-3,webview` alone gains the two window tests, which is the gate compiling away where a purchase cannot be made
