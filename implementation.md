...
- `activation.rs`: 13 tests - the full exit-code table asserted value-by-value, all classified codes distinct and disjoint from 0/1/2, `machine_detail` contents for cooldown/funds/sold-out, no detail line for unclassified failures or for a node-reported shortfall with unknown amounts, a price-only shortfall labelled as excluding gas on both surfaces, a malformed contract classified as terminal rather than retryable, `lowest_token` picking the minimum of an unsorted owned set, the token-scoped fast path reusing the requested token's own session when another token's is newer, and `TxError → HeadlessError` code mapping
...
- `cargo test -p rub3-wrapper --no-default-features --features tier-3,headless --lib`: 102 pass
