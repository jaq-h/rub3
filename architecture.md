...
- **Its own code is verified before its answer is believed.** One address and one masked hash, both frozen into the wrapper at pack time; the deployer is trusted for nothing. The registry publishes the distinct immutable layouts in use so an agent can compute a hash before it has a record to look one up in - one table today, shared by `Rub3Access` and `Rub3Subscription`. **The wrapper reads that list bounded and tries a bounded number of it**, since how many tables exist is the owner key's to choose while the read and each surviving candidate's `record` call sit on the path that spends money: it asks `offsetTableWindow(0, MAX_CANDIDATE_OFFSET_TABLES)` and holds the same cap over the lookups, because a node need not honour the window it was asked for. So neither what a compromised owner key can put on the wire nor the round trips a purchase makes grows with what that key publishes. Past the bound a candidate is unread or untried, which says what a dropped one says. Latency only - an unbounded read could never produce a wrong verdict, only a slow one.
