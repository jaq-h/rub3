...
**Where the fee's scope ends, stated deliberately.** The fee is charged on value that arrives *through* the contract's payment functions. Value that reaches a licence contract any other way - a direct ERC-20 `transfer`, a `selfdestruct` beneficiary, a coinbase payout - is never accrued against, and `withdraw` / `withdrawToken` release it in full to the developer. That is a reasoned position on what the fee means, not an oversight: it is an economic argument rather than a technical lock. A developer can already take payment entirely off-chain and mint free licences, so charging on unaccounted balance would close nothing while taking a cut of mistaken transfers and airdrops, which are not revenue. The fee buys distribution, verification, and liquidity, priced so routing around it costs more than paying it, with the registry and marketplace listing as the carrot.
