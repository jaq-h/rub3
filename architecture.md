...
- **Its own code is verified before its answer is believed.** One address and one masked hash, both frozen into the wrapper at pack time; the deployer is trusted for nothing. `offsetTables()` publishes the distinct immutable layouts in use so an agent can compute a hash before it has a record to look one up in - one table today, shared by `Rub3Access` and `Rub3Subscription`. The wrapper tries a bounded number of them, because each surviving candidate costs a sequential round trip on the path that spends money; past the bound a candidate is untried, which says what a dropped one says.
